...
Finally, we look at `prism`, `bokeh`, and `bucket` (pronounced *bouquet*), three functions for visualizing co-clusters of proportional features. We mention these plot functions together because they share some key similiarities. First, these functions are "index-naive" and plot all $\rho$ in the `@matrix` slot of the `propr` object. Second, these functions label the feature pairs where both constituents belong to the same cluster (with the total number of co-clusters toggled by the argument `k`). Third, these functions return a vector of cluster memberships in the order that the features appear in the `propr` object.
...
The `bokeh` function plots the feature pairs across the individual variances of their constituent log-ratio transformed features. For clarity of visualization, the figure projects the data on a log-fold scale. Therefore, highly variable co-clusters appear in the bottom-left of the figure while lowly variable co-clusters appear in the top-right, and highly proportional pairs tend to aggregate around the $y=x$ diagonal. The user can retrieve the table used to generate the `prism` and `bokeh` plots by passing the `propr` object through the `slate` function.
