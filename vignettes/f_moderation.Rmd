...
To calculate a moderated $F$-statistic from $\theta_d$, we fit the data to an empirical Bayes model (via `limma::eBayes`) with underlying mean-variance modeling (via `limma::voom`). Conventionally, for per-gene (i.e., DE) analysis (and also for VLR weighting), moderation and modeling is done for individual genes. However, for per-ratio (i.e., $\theta_d$) analysis, moderation and modeling is done for gene ratios. To do this, we must select an arbitrary reference, $z$, for which to define a hierarchical model (effectively calculating `limma::voom` on log-ratio transformed data, re-scaled as pseudo-counts). As a consequence, the moderation of the $F$-statistic depends on the chosen reference (although the unmoderated $F$-statistic does not).

In analogy to the clr, we call $g(\mathbf{x_i})$ the geometric mean of the i-th composition (i.e., sample vector). Thus, for $N$ samples, the grand mean of all geometric means is $\Gamma := \frac{\sum_i^N g(\mathbf{x_i})}{N}$. We can now turn the i-th scaled composition (i.e., $\frac{\mathbf{x_i}}{g(\mathbf{x_i})}$) into pseudo-counts using $\Gamma * \frac{\mathbf{x_i}}{g(\mathbf{x_i})}$. It is a matrix of these pseudo-counts that we use for empirical Bayes moderation with underlying mean-variance modeling.
