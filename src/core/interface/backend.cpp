...
#include "../../include/kernels/cuda/dispatch/backend.cuh"


// [[Rcpp::export]]
double propr::wtmRcpp(Rcpp::NumericVector x, Rcpp::NumericVector w) {
    double result;
    // No CUDA implementation provided for wtmRcpp in dispatch.cuda
...
double propr::wtvRcpp(Rcpp::NumericVector x, Rcpp::NumericVector w) {
    double result;
    // No CUDA implementation provided for wtvRcpp in dispatch.cuda
...
Rcpp::NumericMatrix propr::corRcpp(Rcpp::NumericMatrix X) {
    ...
    if (propr::is_gpu_backend()) {
        ...
        if (err != cudaSuccess) {
            ...
            // Assuming cuda::corRcpp is available and takes X, result, context
            // If only a fallback is available in cuda::corRcpp, this will hit the stop there
