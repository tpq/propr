...
namespace host_profiler_detail {
...
void set_host_enabled(bool on) {
    host_profiler_detail::g_host_enabled.store(on, std::memory_order_relaxed);
}
bool host_enabled() {
    return host_profiler_detail::g_host_enabled.load(std::memory_order_relaxed);
}

std::vector<host_record> consume_host_records() {
    std::vector<host_record> out;
    out.swap(host_profiler_detail::g_host_records);
    return out;
}

void record_host_profile(std::string name, double milliseconds) {
    host_profiler_detail::g_host_records.push_back(host_record{std::move(name), milliseconds});
}
...
void start_host_timer(const char* name) {
    ...
    if (!enabled) {
        host_profiler_detail::g_host_timers.erase(name);
        return;
    }
    host_profiler_detail::g_host_timers[name] = host_profiler_detail::host_timer_state{
        true,
        true,
        0.0,
...
void pause_host_timer(const char* name) {
    if (name == nullptr) return;
    auto it = host_profiler_detail::g_host_timers.find(name);
    if (it == host_profiler_detail::g_host_timers.end()) return;
    auto& timer = it->second;
    if (!timer.enabled || !timer.running) return;
    auto now = std::chrono::high_resolution_clock::now();
    timer.accumulated_ms += host_profiler_detail::elapsed_ms(timer.start, now);
    timer.running = false;
}
...
void stop_host_timer(const char* name) {
    ...
    auto timer      = it->second;
    host_profiler_detail::g_host_timers.erase(it);
