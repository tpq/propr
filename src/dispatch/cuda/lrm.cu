...
propr::dispatch::cuda::lrm_basic(NumericVector& out, NumericMatrix &Y, propr::propr_context context) {
    propr::runtime::with_precision([&](auto tag) {
        ...
        Real* d_mean_log;
        PROPR_CUDA_CHECK(cudaMalloc(&d_mean_log, static_cast<size_t>(N_genes) * sizeof(Real)));

        Real* d_mean;
        PROPR_CUDA_CHECK(cudaMalloc(&d_mean, N_pairs * sizeof(Real)));
...

        {
            ...
            propr::detail::cuda::lrm_basic_phase_1<Real, Config><<<grid1, block1, 0, context.stream>>>(
                d_Y, stride, d_mean_log, N_samples, N_genes
            );
            PROPR_CUDA_CHECK(cudaGetLastError());

            propr::detail::cuda::lrm_basic_phase_2<Real, Config><<<grid2, block2, 0, context.stream>>>(
...
propr::dispatch::cuda::lrm_weighted(NumericVector& out,
    ...
    propr::runtime::with_precision([&](auto tag) {
        ...
        offset_t stride_Y; Real* d_Y = RcppMatrixToDevice<Real>(Y, stride_Y);
        offset_t stride_W; Real* d_W = RcppMatrixToDevice<Real>(W, stride_W);
        ...
        {
            PROPR_PROFILE_CUDA("kernel", context.stream);
            propr::detail::cuda::lrm_weighted<Real, Config><<<gridDim, blockDim, 0, context.stream>>>(
...
propr::dispatch::cuda::lrm_alpha(NumericVector& out,
    ...
    propr::runtime::with_precision([&](auto tag) {
        ...
        Real* d_Y; Real* d_Yfull;
        offset_t stride_Y    ; d_Y     = RcppMatrixToDevice<Real>(Y, stride_Y);
        offset_t stride_Yfull; d_Yfull = RcppMatrixToDevice<Real>(Yfull, stride_Yfull);

        Real* d_partials;
        PROPR_CUDA_CHECK(cudaMalloc(&d_partials, static_cast<size_t>(N_genes) * sizeof(Real)));

        Real* d_means;
        PROPR_CUDA_CHECK(cudaMalloc(&d_means, N_pairs * sizeof(Real)));
        ...
        {
            PROPR_PROFILE_CUDA("kernel", context.stream);
            propr::detail::cuda::lrm_alpha_phase_1<Real, Config><<<grid1, block1, 0, context.stream>>>(
                d_Y, stride_Y, d_Yfull, stride_Yfull, N1, NT, static_cast<Real>(a), d_partials, N_genes
            );
            PROPR_CUDA_CHECK(cudaGetLastError());

            propr::detail::cuda::lrm_alpha_phase_2<Real, Config><<<grid2, block2, 0, context.stream>>>(
                d_partials, d_means, N_genes
            );
            PROPR_CUDA_CHECK(cudaGetLastError());
            PROPR_STREAM_SYNCHRONIZE(context);
        ...
        PROPR_CUDA_CHECK(cudaFree(d_partials));
...
propr::dispatch::cuda::lrm_alpha_weighted(NumericVector& out,
    ...
    propr::runtime::with_precision([&](auto tag) {
        ...
        offset_t stride_Y    ; d_Y     = RcppMatrixToDevice<Real>(Y, stride_Y);
        offset_t stride_W    ; d_W     = RcppMatrixToDevice<Real>(W, stride_W);
        offset_t stride_Yfull; d_Yfull = RcppMatrixToDevice<Real>(Yfull, stride_Yfull);
        offset_t stride_Wfull; d_Wfull = RcppMatrixToDevice<Real>(Wfull, stride_Wfull);
        ...
        {
            PROPR_PROFILE_CUDA("kernel", context.stream);
            propr::detail::cuda::lrm_alpha_weighted<Real, Config><<<gridDim, blockDim, 0, context.stream>>>(
                d_Y    , stride_Y,
                d_Yfull, stride_Yfull,
                d_W    , stride_W,
                d_Wfull, stride_Wfull,
                N1, NT, static_cast<Real>(a), d_means, N_genes
            );
            PROPR_STREAM_SYNCHRONIZE(context);
        }
