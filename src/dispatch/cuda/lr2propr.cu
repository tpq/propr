...
void dispatch::cuda::lr2vlr(Rcpp::NumericMatrix &out, NumericMatrix& lr, propr_context context) {
    Rcpp::stop("Not implemented in CUDA for lr2vlr.");
    // Calculate variation matrix
    NumericMatrix x = clone(lr);
}

void dispatch::cuda::lr2phi(Rcpp::NumericMatrix &out, NumericMatrix& lr, propr_context context) {
...
void dispatch::cuda::lr2rho(Rcpp::NumericMatrix &out, NumericMatrix& lr, propr_context context) {
    Rcpp::stop("Not implemented in CUDA for lr2rho.");
    NumericMatrix x = clone(lr);
    // NumericMatrix mat = lr2vlr(x);
}

void dispatch::cuda::lr2phs(Rcpp::NumericMatrix &out, NumericMatrix &lr, propr_context context) {
