    ...
    namespace detail {
        namespace cuda {
            ...
            compute_odd_ratio(cub::ScanTileState<int4> tile_state,
                ...
                // Phase 1: Initialization (only needed threads participate)
                const int init_blocks_needed = cub::DivideAndRoundUp(gridDim.x, blockDim.x);
                const int total_init_threads = init_blocks_needed * blockDim.x;
                ...
                for (int k = blockDim.x * bid + tid; k < total_pairs; k += stride) {
                    if (global_tid < total_init_threads) tile_state.InitializeStatus(blockDim.x);
                    ...
                    if(gridDim.x > 1){
                        if (tile_idx == 0) {
                            ...
                            if (tid == 0) {
                                int4 exclusive_prefix = prefix(blk_acc);
                                acc->x = scan_op(exclusive_prefix, blk_acc).x;
                                acc->y = scan_op(exclusive_prefix, blk_acc).y;
                                acc->z = scan_op(exclusive_prefix, blk_acc).z;
                                acc->w = scan_op(exclusive_prefix, blk_acc).w;
                            }
                        }
                    } else {
